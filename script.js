...
const input = document.getElementById('roof-photo');
const canvas = document.getElementById('roof-canvas');
const ctx = canvas.getContext('2d');

// Панель PNG
const panel = new Image();
panel.src = 'panel_overlay.png';

// Позиция и угол панели
let panelX = 0, panelY = 0;
let panelWidth = 200, panelHeight = 100;
let angle = 0;
...
input.addEventListener('change', (e) => {
    ...
    img.onload = () => {
        ...
        panelWidth = img.width / 4;
        panelHeight = panelWidth * 0.5;
        panelX = img.width / 2 - panelWidth / 2;
        panelY = img.height / 2 - panelHeight / 2;
        ...
        function draw() {
            ...
            ctx.drawImage(panel, -panelWidth / 2, -panelHeight / 2, panelWidth, panelHeight);
            ctx.restore();
        }
        ...
        canvas.onmousedown = (e) => {
            ...
            // Преобразуем координаты мыши с учётом масштабирования canvas
            ...
            if (mouseX > panelX && mouseX < panelX + panelWidth &&
                ...
                isDragging = true;
                offsetX = mouseX - panelX;
                offsetY = mouseY - panelY;
        ...
        // Вращение колёсиком мыши
        canvas.onwheel = (e) => {
            angle += e.deltaY > 0 ? 5 : -5; // +5 или -5 градусов
            draw();
            e.preventDefault();
        };
    };
...
// === РАСЧЁТ ПО РЕГИОНАМ РОССИИ ===

// Константы панели
const PANEL_POWER_W = 450; // мощность панели
const SYSTEM_LOSS = 0.85;
const TARIFF = 5.5; // руб/кВт·ч

let regions = {}; // данные из JSON
...
if (svgMap) {
  const regionsPaths = svgMap.querySelectorAll('.region');
  ...
  regionsPaths.forEach(path => {
    path.addEventListener('click', () => {
      ...
      // Берём количество панелей из ползунка
      const count = parseInt(document.getElementById('count').value);

      // Расчёт
      const pvout = data.pvout;
      const systemPower = (PANEL_POWER_W * count) / 1000; // кВт
      const yearlyGen = pvout * systemPower * SYSTEM_LOSS;
      const yearlySavings = yearlyGen * TARIFF;

      // Обновляем вывод
      resultsBox.innerHTML = 
        <p><strong>Регион:</strong> ${data.name}</p>
